...
function initSigma(config) {
    ...
    if (config.sigma && config.sigma.drawingProperties)    
        ...
        labelThreshold: 10,
        // *** FIX 1: Set edge type to "line" for straight edges ***
    ...
    dataReady = function() {//This is called as soon as data is loaded
        ...
        // >>> START OF THE FIX FOR UPSIDE-DOWN MAP <<<
        // Iterate over all nodes and multiply the Y-coordinate by -1
        ...
        // *** FIX 2: Bind to clickNode for clearer node interaction ***
        a.bind("clickNode", function (a) {
...
// *** FIX 3: Added the missing createList function ***
function createList(a) {
    var b = [];
    // Iterate through the neighbor list object 'a'
    for (var c in a) {
        // Get the node object from the graph index using the ID 'c'
        var d = sigInst._core.graph.nodesIndex[c];
        // Create the HTML list item with a link that calls nodeActive() on click
        b.push('<li class="' + (d.color) + '"><a href="#' + d.label + '" onmouseover="sigInst._core.plotter.drawHoverNode(sigInst._core.graph.nodesIndex[\'' + d.id + '\'])" onclick="nodeActive(\'' + d.id + '\')" onmouseout="sigInst.refresh()">' + d.label + "</a></li>");
    }
    return b;
}
